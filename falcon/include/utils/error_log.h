...
#define FALCON_ELOG_ERROR_EXTENDED(errorCode, errorFormat, ...)                             \
    FALCON_ELOG_INTERNAL(ERROR, "%c " errorFormat, ((char)(errorCode) + 64), ##__VA_ARGS__)
...
#define FALCON_ELOG_WARNING_EXTENDED(errorCode, errorFormat, ...)                           \
    FALCON_ELOG_INTERNAL(WARNING, "%c " errorFormat, ((char)(errorCode) + 64), ##__VA_ARGS__)
...
#define FALCON_ELOG_LOG_EXTENDED(logFormat, ...)                                            \
    FALCON_ELOG_INTERNAL(LOG, logFormat, ##__VA_ARGS__)
...
#define FALCON_ELOG_FATAL_EXTENDED(errorCode, errorFormat, ...)                             \
    FALCON_ELOG_INTERNAL(FATAL, "%c " errorFormat, ((char)(errorCode) + 64), ##__VA_ARGS__)
...
#define FALCON_ELOG_THREAD_SAFE_EXTENDED(logFormat, ...) \
    fprintf(stderr, "[MT-LOG][%s:%d] " logFormat "\n", __FILE__, __LINE__, ##__VA_ARGS__)
